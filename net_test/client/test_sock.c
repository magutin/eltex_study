...
void* run_ping(){
	...
	for(int i = 0;i<settings.cnt_ptk;i++){
		...
		fprintf(fping,"\tping %s  %ld(%ld) bytes of data\tpackage num %u\n",d_ip_str,sizeof(snd_ptk)-28,sizeof(snd_ptk),icmp_hdr.SEQ_NUM);
...
#define FREE_MEM free(snd_ptk);free(rcv_ptk)
	...
	fprintf(fudpseq,"   Package send=%d\n   Package reciev=%d\n   Len package=%d (20 bytes IP HEADER,8 bytes UDP HEADER)\n",sent,recv,packet_size);	
	get_time_string(time_string,sizeof(time_string));
...
#define FREE_MEM free(snd_ptk);free(rcv_ptk)
    ...
    struct iphdr *ip_rcv = (struct iphdr *) rcv_ptk;
    struct udphdr *udp_rcv = (struct udphdr *) (rcv_ptk + sizeof (struct iphdr));
    struct data_ptk *data_rcv = (struct data_ptk*) (rcv_ptk + sizeof (struct iphdr) + sizeof(struct udphdr));
    ...
    ip->daddr = d_ip;
   	...
   	struct data_ptk *data = (struct data_ptk*) (snd_ptk + sizeof (struct iphdr) + sizeof(struct udphdr));
	...
	while(1){
		...
		if(sucsess==2){
			...
			fprintf(fudpload,"   Sucsess test num=%d\n   Len package=%d (20 bytes IP H,8 bytes UDP H)\n   Good time sleep=%ld mcs\n",(count_test-1),packet_size,time_sleep);
			break;
		}
